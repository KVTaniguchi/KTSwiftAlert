...
extension ExampleViewController: UICollectionViewDelegate, UICollectionViewDelegateFlowLayout, UICollectionViewDataSource {
    func layoutExampleButtons() {
        ...
        if let cv = collectionView {
            ...
            sv.axis = .vertical
            sv.wrap(in: view, with: InsetConstraints(insets: UIEdgeInsets(top: 44, left: 0, bottom: 0, right: 0) , priority: UILayoutPriority.defaultHigh))
        }
    }
    ...
    func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
        ...
        if let label = exampleSectionLabels?[indexPath.section][indexPath.row] {
            ...
            label.wrap(in: view, with: InsetConstraints(insets: UIEdgeInsets.zero, priority: UILayoutPriority.defaultHigh))
        }
        return view
    }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if let button = exampleButtons?[indexPath.section][indexPath.row] {
            button.wrap(in: cell, with: InsetConstraints(insets: UIEdgeInsets.zero, priority: UILayoutPriority.defaultHigh))
        }
...
extension ExampleViewController {
    ...
    var customView: UIView {
        ...
        sv.spacing = 20
        sv.wrap(in: customView, with: InsetConstraints(insets: UIEdgeInsets(top: 20, left: 20, bottom: 20, right: 20), priority: UILayoutPriority.defaultHigh))
...
class ExampleCustomButtons: UIView, AlertButtonContainer {
    ...
    override init(frame: CGRect) {
        ...
        sv.wrap(in: self, with: InsetConstraints(insets: UIEdgeInsets.zero, priority: UILayoutPriority.defaultHigh))
    }
